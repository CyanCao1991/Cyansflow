---
name: cyancao-sdd-create
description:把模糊需求转化为可评审的系统需求文档（SDD），并生成带交互的视觉原型页面。当用户说"写个需求文档"、"帮我出PRD"、"这个功能怎么写需求"、"我有个想法想落地"、"把这个需求整理成文档"、"需求评审要用的PRD"、"生成原型"、"做个Demo"、"做个演示页面"，或者用户描述了一段功能但没有结构化时，使用这个 Skill。
也适用于：用户上传了原始需求描述/会议纪要/聊天截图并要求整理成PRD；用户说"PRD"、"产品需求"、"需求文档"、"功能说明书"、"原型"、"Demo"等关键词；用户要求对已有PRD进行补全、优化、查漏补缺；用户要求生成可交互的演示页面。
...
严格按照以下结构输出。读取 `references/cyancao-sdd-template.md` 获取完整模板，以下是结构概览：
...
- **Mermaid 流程图最佳实践**：
  - 使用 `<pre class="mermaid">` 标签包裹代码，不要用 markdown 代码块或 script 标签
  - 流程图默认使用 `flowchart LR`（横向布局），更符合阅读习惯
  - 状态转换图使用 `stateDiagram-v2` + `direction LR`（横向布局）
  - 系统架构图使用 `graph TB`（纵向布局）展示层级关系
- **数据字典用表格**：字段名、类型、必填、说明、示例值
- **界面交互标注每个元素**：控件类型、默认值、校验规则、操作反馈

### 演示Demo生成

除了PRD文档，还支持根据PRD生成可操作的演示Demo页面：

- **数据概览页**：展示核心指标卡片、趋势图表、业务流程图
- **功能模块页**：补货建议列表、审核管理、订单管理、状态跟踪
- **交互能力**：审核通过/驳回、查看详情、状态跟踪查询
- **技术实现**：纯HTML/CSS/JavaScript，无需后端依赖
